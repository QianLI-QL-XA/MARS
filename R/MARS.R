...
#' sol <- MARS(X, Lambdapath = seq(1, 0.5, -0.1), stopmethod = "fix",
#'             fixnumber = 3L, maxiter = 5, printmain = FALSE)
#' sol$Omegapath
#' @export
MARS <- function(X = NULL, stoptol = 1e-04, Lambdapath = seq(1, 0.5, -0.1), maxiter = 10,
    ...
    if (nrow(X) <= ncol(X)) {
        warning("MARS is designed for the high-dimensional setting where the sample dimension p is much greater than the sample size n.")
    }
    ...
    pathsolution <- MARSc(X, stoptol, Lambdapath, stopmethod, maxiter, printmain,
