...
Rcpp::List PMEASc(arma::mat X, double stoptol, arma::vec Lambdapath,  std::string calmethod,  std::string stopmethod,  unsigned int maxiter,  bool printyes,  bool printyessub, double sigma, int numlam, bool maxlambdacheck){
  ...
  // the full matrix S = A * A.t() is only needed by the eADMM method; for the
  // other methods its diagonal and the maximum lambda are computed on the fly.
  arma::vec Sdiag(p);
  double maxlambda;
  if (calmethod == "eADMM") {
      S = A * A.t();
      Sdiag = S.diag();
      maxlambda = findmaxlambda(S);
  } else {
      maxlambda = findmaxlambdaA(A, Sdiag);
  }
  ...
  if (printyes){
      ...
      if (calmethod == "iADMM") {
          Rcout << " method = iADMM starts:" << endl;
      } else if (calmethod == "SSNAL") {
        Rcout << " method = SSNAL starts:" << endl;
      } else if (calmethod == "eADMM") {
        Rcout << " method = eADMM starts:" << endl;
      }
  }
  if (maxlambdacheck){
  ...
  if (printyes) {
      Rcout << "*********************************************************************" << endl;
      Rcout << " Iter|   lambda   |  primobj   dualobj     gap   | primfeas  dualfeas  eta   | time   nnzoff" << endl;
  }
  ...
  for (int iterpath = 0; iterpath < pathlength; iterpath++)
  {
    ...
    if (printyes) {
      ...
      Rcout << "   " << dec << iterpath << " | "  << fixed << lambda << " | " << scientific << primobj << "  " << dualobj << "  " << gap \
  ...
  if (printyes){
    ...
    Rcout << " total time for calculating solution path: " << fixed << timepathwhole << "(s)" << endl;
    Rcout << "*********************************************************************" << endl;
  }
               ...
               double stoptol, int p, int n)
{
  ...
  for (int itersub = 0; itersub < maxitersub; itersub++) {
    ...
    // note: the original implementation contained a block "if (itersub > 1000)"
    // with additional stagnation checks. Since maxitersub is at most 30 here,
    // that branch was unreachable dead code and has been removed.
            ...
            double &dualobj, double &gap, double &primfeas, double &dualfeas, double &eta, int &nnzOmega, mat Ip)
{
    ...
    if (printyessub) {
        Rcout << " ---------------------------------------" << endl;
        Rcout << " subproblem results printing start:" << endl;
        Rcout << " iter |    primobj  dualobj     gap   |   primfeas      dualfeas        eta     |   sigma   " << endl;
        Rcout << "   " << dec << iter << " | " << scientific << primobj << "  " << dualobj << " "<< gap << " | " \
         << primfeas << " " << dualfeas << " " << eta << " " << sigma << endl;
    }
    if (breakyes) {
...
// find the maximum lambda from the full matrix S (used by eADMM)
...
// find the maximum lambda directly from A (S = A * A.t() is never formed),
// returning the diagonal of S in Sdiag. Used by SSNAL and iADMM.

double findmaxlambdaA(mat A, vec& Sdiag){
  int p = A.n_rows;
  double elementS, maxlambda = 0;
  for (int i = 0; i < p; i++){
    Sdiag(i) = dot(A.row(i), A.row(i));
  }
  for (int i = 0; i < p - 1; i++){
    for (int j = i + 1; j < p; j++){
      elementS = dot(A.row(i), A.row(j));
      elementS = 0.5 * fabs(elementS / Sdiag(i) + elementS / Sdiag(j));
      if (elementS > maxlambda){
        maxlambda = elementS;
      }
    }
