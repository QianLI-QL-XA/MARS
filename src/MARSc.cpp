...
Rcpp::List MARSc(arma::mat X, double stoptol, arma::vec Lambdapath, std::string stopmethod,  unsigned int maxiter,  bool printyes,  bool printyessub, double sigma, int numlam, bool maxlambdacheck){
  ...
  if (pathlength == 0){
    Rf_warning("All entries of Lambdapath are not smaller than the maximum lambda; no solution is computed.");
    return Rcpp::List::create(Named("Omegapath") = Rcpp::List(),
                              Named("Lambdapath") = Lambdapath,
                              Named("timepath") = arma::vec());
  }
  ...
  for (int iterpath = 0; iterpath < pathlength; iterpath++)
  {
    ...
    arma::mat gradP = 0.5 * ( (Omega * A) * A.t() + A * (A.t() * Omega) );
    gradP.diag() -= 1.0;
    ...
    if (printyes){
      if (etaorg < stoptol){
        Rcout << "etaorg = " << scientific << etaorg << ", which already < stoptol, solution is as same as the previous one " << endl;
      }else{
        Rcout << " Iter|   etaorg   |  primobjsub   dualobjsub     gapsub   | primfeassub  dualfeassub  etasub   | time   nnzoff" << endl;
      }
      if (iterpath == 0){
        timeiter = itertime.toc();
        numAS = 1;
        Rcout << "   " << dec << numAS << " | " << scientific <<  etaorg << " | " << primobj << "  " << dualobj << "  " << gap \
    ...
    while (etaorg > stoptol && numAS < 10){
      ...
      if (nonzeronumbernewIndex > 5 * nonzeronumberIndex && nonzeronumberIndex != 0){
        ...
        // keep the largest nonzeronumberIndex entries (bounded to avoid an
        // out-of-range access when the active set is already large)
        arma::uword nkeep = std::min<arma::uword>(nonzeronumberIndex, sortindex.n_elem - 1);
        if (nkeep > 0){
          newJ = sortindex(arma::span(0, nkeep - 1));
    ...
    if (printyes) {
      Rcout << " total time in this calculation process: " << fixed << timeiter << "(s)" << endl;
    }
    if (stopmethod == "bigs"){
      if (nnzOmega > (numlam * n) || numAS > 4){
        if (printyes){
          Rcout << " lambda small enough" << endl;
        }
  ...
  if (printyes){
    double timepath_total = pathtime.toc();
    Rcout << "\n total time for calculating solution path: " << fixed << timepath_total << "(s)" << endl;
    Rcout << "\n *********************************************************************" << endl;
  }

  // convert the stored field of sparse matrices into an R list explicitly
  int npath = Omegapath.n_elem;
  Rcpp::List OmegapathOut(npath);
  for (int i = 0; i < npath; i++){
    OmegapathOut[i] = Omegapath(i);
  }
  return Rcpp::List::create(Named("Omegapath") = OmegapathOut,
                            Named("Lambdapath") = Lambdapath,
...
void MARSmainc(arma::mat A, double lambda, double stoptol, int maxiter, arma::uvec Index, arma::umat subindex, arma::mat &Omega, arma::mat &Y, int p, int n, double sigma, bool printyessub, double &primobj,\
  ...
  if ((max(primfeas, dualfeas) < 500 * max(1e-6, stoptol)) && (eta < stoptol) && (fabs(gap) < stoptol)){
    breakyes = true;
  }

  if (printyessub){
    Rcout << " ---------------------------------------" << endl;
    Rcout << " subproblem results printing start:" << endl;
    Rcout << " iter|  objprimal    objdual      gap    |  primfeas    dualfeas      eta       sigma  " << endl;
    Rcout << "   " << dec << iter << " | " << scientific << primobj << "  " << dualobj << " "<< gap << " | " \
...
// find the maximum lambda, using the identity S = A * A.t() so that only the
// diagonal of S and the off-diagonal entries needed for the comparison are
// computed on the fly. This avoids allocating the full p x p matrix S, which
// is a decisive memory saving in the high-dimensional setting.
double findmaxlambda(arma::mat A, arma::vec& vecdiagS){
  int p = A.n_rows;
  double elementS, maxlambda = 0;
  // diagonal entries S(i,i)
  for (int i = 0; i < p; i++){
    vecdiagS(i) = dot(A.row(i), A.row(i));
  }
  // off-diagonal entries S(i,j)
  for (int i = 0; i < p - 1; i++){
    for (int j = i + 1; j < p; j++){
      elementS = dot(A.row(i), A.row(j));
      elementS = 0.5 * fabs(elementS / vecdiagS(i) + elementS / vecdiagS(j));
